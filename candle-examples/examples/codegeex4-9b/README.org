...
#+begin_src shell
  cargo run --example codegeex4-9b --release -- --prompt "please write a insertion sort in rust" --sample-len 300
#+end_src
...
*** Input
#+begin_src shell
  cargo run  --release -- --prompt 'please write a FFT in rust' --sample-len 500 --cache /root/autodl-tmp 
#+end_src
