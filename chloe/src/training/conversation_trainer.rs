...
pub async fn prepare_conversation_training_data(config: &TrainingConfig, conversations_path: &str) -> Result<()> {
    ...
    for entry in WalkDir::new(conversations_path).into_iter().filter_map(|e| e.ok()) {
    ...
    for conv in &conversations {
        let text = format!("<|user|>{}<|assistant|>{}", conv.user, conv.assistant);
    ...
    let safetensors_path = training_dir.join("conversation_training_data.safetensors");
    ...
    let metadata_path = training_dir.join("conversation_metadata.json");
    fs::write(&metadata_path, serde_json::to_string_pretty(&metadata)?)?;

    println!("Conversation training data prepared in: {}", training_dir.display());
    Ok(())
}